 ...
 * @return void*  NULL (complete parsing).
 */
void*ContainerIntersectDelIf(t_container*pContainerA, t_container*pContainerB, t_ptfVV pIntersectFunc, 
								t_ptfVV pCallbackFunc, void*pParam){
	...
	while(pParseA && pParseB){
		...
		while(pParseA && pParseB){
			if(pIntersectFunc(pParseA->m_pItem, pParseB->m_pItem)){
				...
				if(pCallbackFunc) pCallbackFunc(pParam, pParseB->m_pItem);
