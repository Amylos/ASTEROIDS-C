...
t_scene*SceneNew(int iWidth, int iHeight, SDL_Renderer*pRenderer, TTF_Font*pFont){
  ...
  pScene->m_pEngagedShip = ShipEngage(ContainerGetback(pScene->m_pShips), pScene->m_spaceArea.w/2-SCENE_SHIP_SIZE/2,
...
t_scene*SceneKeydown(t_scene*pScene, SDL_KeyboardEvent*pEvent){
    ...
    case SDLK_RIGHT:
         ShipTurning(pScene->m_pEngagedShip, TURN_RIGHT);
        break;
    case SDLK_LEFT:
         ShipTurning(pScene->m_pEngagedShip, TURN_LEFT);
        break;
    case SDLK_UP:
         ShipThrusting(pScene->m_pEngagedShip, 1);
        break;
    case SDLK_SPACE:
        break;
    default:
...
t_scene*SceneKeyup(t_scene*pScene, SDL_KeyboardEvent*pEvent){
    ...
    switch (pEvent->keysym.sym){
    case SDLK_RIGHT:
    	 ShipTurning(pScene->m_pEngagedShip, TURN_NONE);
		break;
    case SDLK_LEFT:
        ShipTurning(pScene->m_pEngagedShip, TURN_NONE);
        break;
    case SDLK_UP:
         ShipThrusting(pScene->m_pEngagedShip, 0);
        break;
    case SDLK_SPACE:
